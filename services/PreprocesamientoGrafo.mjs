import mapa from './GrafoSinProcesar.json' assert { type: 'json' };
...
mapa['elements'].forEach(elementoMapa => {
    if (elementoMapa['type'] == 'node') {
        ...
        conjuntoPuntos.push([elementoMapa['lat'], elementoMapa['lon'], elementoMapa['id']]);
...
await fsPromises.writeFile('./GrafoProcesado.json', JSON.stringify(datosProcesados));